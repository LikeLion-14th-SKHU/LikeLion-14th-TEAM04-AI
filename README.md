...
```mermaid
flowchart TD
    A["컨셉 이미지 3장"] --> B["Haiku가 3장 병렬 채점<br/>① 스펙 반영 ② 브랜드 정합성 ③ 시각 품질<br/>④ 실현 가능성 ⑤ 로고 판독성"]
    B --> C{전원 통과?}
    C -->|Yes| D["3장 점수순 제시"]
    C -->|No| E["탈락분만 fail_reasons 피드백<br/>→ 재생성 1회 → 재채점"]
    E --> F{3장 확보?}
    F -->|Yes| D
    F -->|No| G["탈락 컷 중 최고점으로 보충<br/>(passed=false 표시 유지)<br/>→ 항상 정확히 3장 제시"]
```

- **후보는 항상 정확히 3장** — 크레딧 모델(1번 무료 + 2·3번 잠금 해제)의 전제를 구조로 보장
